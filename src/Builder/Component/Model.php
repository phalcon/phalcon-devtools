    ...
    public function build(): void
    {
        ...
        foreach ($referenceList as $tableName => $references) {
            foreach ($references as $reference) {
                ...
                $initialize[] = $snippet->getRelation(
                    ...
                    $entityNamespace . ucwords($tableName, '_-'),
                    $this->getFieldName($columns[0]),
                    "['alias' => '" . ucwords($tableName, '_-') . "']"
                );
            }
        }
        ...
        foreach ($db->describeReferences($this->modelOptions->getOption('name'), $schema) as $reference) {
            ...
            $initialize[] = $snippet->getRelation(
                ...
                $this->getFieldName($refColumns[0]),
                "['alias' => '" . ucwords($reference->getReferencedTable(), '_-') . "']"
            );
        }
            ...
            try {
                ...
                if ($useSettersGetters) {
                    foreach ($fields as $field) {
                        ...
                        $methodName = ucwords($field->getName(), '_-');
        ...
        if ($this->isConsole()) {
            ...
            $this->notifySuccess(sprintf($msgSuccess, ucwords($this->modelOptions->getOption('name'), '_-')));
        }
    }
